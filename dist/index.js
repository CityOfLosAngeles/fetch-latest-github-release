...
process.env.INPUT_GITHUB_REPOSITORY =
    process.env.NODE_ENV === 'test'
        ...
        : process.env.INPUT_GITHUB_REPOSITORY;
const GITHUB_ACTION_OUTPUT = process.env.GITHUB_OUTPUT || './__tests__/github_output_test.txt'; // see more information here https://docs.github.com/en/actions/using-workflows/workflow-commands-for-github-actions#example-of-setting-an-output-parameter
...
const [owner, repo] = repoInput
    ...
    : process.env.INPUT_GITHUB_REPOSITORY.split('/', 2);
...
async function appendGHOutputfile(content) {
    try {
        if (GITHUB_ACTION_OUTPUT) {
            await promises.appendFile(GITHUB_ACTION_OUTPUT, content);
        }
    }
    catch (error) {
            ...
            switch (true) {
                ...
                case releaseAttributes[i].includes('author'): // ex. author_url > data.author.url
                    ...
                    github_output =
                        releaseAttributes[i] +
                            '=' +
                            data.author[tempAuthorAttribute] +
                            '\n' +
                            github_output;
                    break;
